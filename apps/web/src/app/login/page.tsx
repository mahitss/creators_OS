...
function LoginContent() {
    ...
    script.onload = () => {
      ...
      if (googleApi?.accounts?.id) {
        googleApi.accounts.id.initialize({
          client_id: process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID || 'vapor-os-client-id.apps.googleusercontent.com',
  ...
  const handleDevOrPasskeyLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    setErrorMsg(null);
    try {
      // Passkey or Mock Google Token Verification
          ...
          {activeTab === 'google' ? (
            <div className="flex flex-col items-center gap-4">
              ...
              <div id="google-signin-btn" className="w-full flex justify-center py-2 min-h-[44px]">
                {/* Fallback button if GIS script is blocked / loading */}
                <button
                  onClick={() => handleDevOrPasskeyLogin({ preventDefault: () => {} } as any)}
                  disabled={isLoading}
            ...
            <form onSubmit={handleDevOrPasskeyLogin} className="space-y-4">
              <div>
...
export default function LoginPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#07090E] flex items-center justify-center text-slate-400">Loading Vapor OS Authentication...</div>}>
      <LoginContent />
    </Suspense>
  );
}
