...
async def list_capabilities(
    ...
    return await capability_registry_service.discover_capabilities(
        db, workspace_id=workspace_id, query=query, cap_type=type, category=category
    )
...
async def list_installations(
    ...
    return await capability_registry_service.list_installations(db, workspace_id=workspace_id)
...
async def list_requests(
    ...
    return await capability_registry_service.list_requests(db, workspace_id=workspace_id)
...
async def create_request(
    ...
    return await capability_registry_service.request_installation(
        db, workspace_id=workspace_id, req=req
    )

@router.post("/capabilities/requests/{request_id}/approve", response_model=CapabilityRequestRead)
async def approve_request(
    request_id: str,
    reviewer: str = Header("admin_reviewer", alias="X-User-Id"),
    db: AsyncSession = Depends(get_db)
):
    """Approves a pending capability installation request."""
    return await capability_registry_service.approve_request(db, request_id=request_id, reviewed_by=reviewer)
...
async def invoke_capability(
    ...
    return await capability_registry_service.invoke_capability(
        db, workspace_id=workspace_id, capability_id=capability_id, req=req, organization_id=organization_id
    )
