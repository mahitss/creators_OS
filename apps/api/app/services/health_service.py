...
async def check_database_connection(session: Optional[AsyncSession]) -> bool:
    if session is None:
        return False
    try:
        ...
        logger.warning(f"Database health check probe failed: {err}")
        return False

async def check_redis_connection(redis_url: str) -> bool:
    if not redis_url or redis_url == "mock" or "redis" not in redis_url.lower():
        return False
    try:
        import redis.asyncio as aioredis
        client = aioredis.from_url(redis_url, socket_timeout=1.0)
        pong = await client.ping()
        await client.aclose()
        return bool(pong)
    except Exception as err:
        logger.debug(f"Redis health check probe: {err}")
        return False

async def get_system_health(session: Optional[AsyncSession], redis_url: str) -> ServiceHealthStatus:
